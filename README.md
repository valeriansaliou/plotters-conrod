...
Also, note that only a single font family (ie. `serif`, `sans-serif`, etc.) and a single font style (ie. `regular`, `bold`, etc.) are supported for text rendering. The reason to that, is that Conrod makes it quite tedious to load fonts and pass them over, so we better off limit this backend API to a single font for simplicity's sake. As well, font transforms are not supported due to the underlying Conrod renderer, which does not seem to support eg. rotations.
